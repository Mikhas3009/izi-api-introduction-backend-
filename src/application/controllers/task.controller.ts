...
export class TaskController {
     ...
     */
    public async addTask(req: TaskRequestInterface, res: Response, next: NextFunction) {
        try {
     ...
     */
    public async setTaskStasus(req: TaskRequestInterface, res: Response, next: NextFunction) {
        try {
    ...
    /**
     * Контроллер для получения задачи по её ID
     *
     * @param req - объект запроса, содержащий id задачи
     * @param res - объект ответа Express
     *
     * @returns Возвращает JSON-ответ с полученной задачей
     * @throws 500 - если не удалось получить задачу
     */
    public async getTaskByID(req: Request, res: Response, next: NextFunction) {
        try {
            const task = await this.taskService.getTaskByID(req);
            res.status(HttpStatus.OK).json(buildResponse(true, "Задача получена", task));
        } catch (err: unknown) {
            next(err);
        }
    }
